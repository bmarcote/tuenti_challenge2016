...
def get_max_sum(matrix, matrix2, ncase=0):
    # We extend the matrix to accomodate the boundary conditions
    # pdb.set_trace()
    # matrix2 responds to a first attempt to resolve it. It has some
    # advantages... but now it is too late to change everything.
    # this is faster as I do not have memory problems (and mi computer neither).
    n,m = matrix.shape
    if n==1 and m==1:
        if matrix[0,0] > 0:
            return 'INFINITY'
    ...
    work_matrix2 = np.zeros((n+1+max(0,n-2), m+1+max(0,m-2)))
    work_matrix[1:n+1,1:m+1] = matrix
    work_matrix2[1:n+1,1:m+1] = matrix2
    # repeating the matrix
    work_matrix[n+1:,1:m+1] = work_matrix[1:n+1-2,1:m+1]#+work_matrix[n,1:m+1]
    work_matrix2[n+1:,1:m+1] = work_matrix2[1:n+1-2,1:m+1]+work_matrix2[n,1:m+1]
    for i in range(m+1, m+1+max(0,m-2)):
        work_matrix2[:,i] = work_matrix2[:,i-m]+work_matrix2[:,m]
        work_matrix[:,i] = work_matrix[:,i-m]#+work_matrix2[:,m]
    ...
    for i in range(1, n+1):
        for i2 in range(1, i):
            if work_matrix2[i,m]-work_matrix2[i-i2,m] > 0:
                return 'INFINITY'
    for j in range(1, m+1):
        for j2 in range(1, j):
            if work_matrix2[n,j]-work_matrix2[n,j-j2] > 0:
                return 'INFINITY'
    # print(work_matrix)
    # Then we need to check everything
    n2,m2 = work_matrix.shape
    for i in range(1, m+1):
        sum_array = np.zeros_like(work_matrix[:,0])
        for j in range(i, max(m+1, m+1+i-2)): #m+1+i-2
            sum_array += work_matrix[:,j]
            # Kadane's alhorithm
            max_local_sum = sum_array[0]
            for k in sum_array[1:]:
                if k >= max_local_sum+k:
                    max_local_sum = k
                else:
                    max_local_sum += k
...
def do_case(index, n_case=0):
    ...
    for i in range(index+1, index+n+1):
        ...
        matrix2[i-index-1,:] = matrix[i-index-1,:]
        if i-index-1 != 0:
            matrix2[i-index-1,:] += matrix2[i-index-2,:]
    for col in range(1, m):
        matrix2[:,col] += matrix2[:,col-1]
    # if n_case == 6:
        # print(matrix)
    return get_max_sum(matrix, matrix2, n_case)
